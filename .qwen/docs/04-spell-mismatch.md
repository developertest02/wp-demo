...
- The `grep` filter removes entries with apostrophes, hyphens, or digits
  (e.g. `won't`, `co-op`, `mp3`) to keep the dictionary simple and uniform
- If `/usr/share/dict/words` is not available, document the required install
  command (`sudo apt-get install wamerican` on Debian/Ubuntu) in the README
...
- The `-a` flag in `wp-spell-mismatch` automates the above
...
|---|---|---|
| 1 | All words in dictionary | No output, exit 0 |
| 2 | One misspelled word `kittne` | Outputs `kittne` |
| 3 | Two misspelled words | Both output, one per line |
| 4 | Empty input | No output, exit 0 |
| 5 | `-a` flag adds word to dictionary | Word is no longer flagged in subsequent run |
| 6 | `-d` flag uses alternate dictionary | Respects the alternate file |

Tests should use a **small controlled test dictionary** (not the full system word list)
for determinism. Create `tests/fixtures/test-dict.txt` with a known small word set.
...
- `echo "kittne" | bin/spell/wp-spell-mismatch` outputs `kittne`
- `echo "the" | bin/spell/wp-spell-mismatch` produces no output
